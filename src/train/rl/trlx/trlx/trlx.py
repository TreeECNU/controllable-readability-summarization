...
def train(  # noqa: C901
    # model_path: 模型路径（可以是 Hugging Face 仓库或本地目录）。
    # reward_fn: 奖励函数，输入为样本、提示和输出，输出为奖励列表，用于在线训练（如 PPO）。
    # dataset: 已废弃的数据集格式，包含样本和奖励的元组，现推荐用 samples 和 rewards。
    # samples: 样本列表（字符串或提示-输出对），用于离线训练。
    # rewards: 样本对应的奖励值列表。
    # prompts: 在线训练时用于生成的提示。
    # eval_prompts: 用于验证的提示。
    # metric_fn: 指标函数，计算生成样本的统计信息。
    # config: 训练配置对象（TRLConfig 类型）。
    # stop_sequences: 停止序列，用于截断生成结果。
    ...
    # 获取训练器，对应的是accelerate_ppo_trainer
