...
class PPOConfig(MethodConfig):
    ...
    def get_advantages_and_returns(
        ...
        # PPO算法的核心步骤
        # 利用GAE公式计算Advantage
        # delta = reward + gamma * next_value - value（时序差分误差）
        # 累加折扣后的delta，得到advantage
    ...
    def loss(
        ...
        # 计算裁剪后的价值估计
        # 将 values 限制在 
        # [old_values - cliprange_value, old_values + cliprange_value] 范围内
        ...
        # 计算有效的token数量
        n = mask.sum()
        # 计算values损失，取两个损失（未裁剪和裁剪后）中的较大者
